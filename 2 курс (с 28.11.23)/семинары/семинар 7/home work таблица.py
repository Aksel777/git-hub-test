...
# в качестве аргумента функцию, вычисляющую элемент по номеру строки и столбца. По умолчанию 
# номер столбца и строки = 9.
...
def print_operation_table(operation, index = 9, columns = 9):
    if index < 3:
        print('ОШИБКА! Размерности таблицы должны быть больше 2!')
        return
    a = [[operation(i, j) for j in range(1, columns + 1)] for i in range(1, index + 1)]
    for i in a:
        print(*[f"{x:>3}".lstrip() for x in i], sep=' ', end='\n')
